...
vi.mock("@/lib/speech/intron-stream", async (importOriginal) => {
  const real = await importOriginal<typeof import("@/lib/speech/intron-stream")>();
  return { ...real, transcribeStream: (...args: unknown[]) => transcribeStream(...args) };
});

const { POST } = await import("@/app/api/voice/transcribe/route");
const { IntronStreamError } = await import("@/lib/speech/intron-stream");
...
describe("upload limits", () => {
  it("keeps the largest recording under Vercel's 4.5 MB request body limit", () => {
...
describe("friendly speech errors", () => {
  ...
  it("has a generic fallback for unknown codes", () => {
    expect(friendlySttError("SOMETHING_NEW")).toMatch(/try again/i);
  });
});
...
describe("POST /api/voice/transcribe", () => {
  ...
  it("returns the transcript for a good recording", async () => {
    transcribeStream.mockResolvedValue({ transcript: " Adebayo bought cement ", msStopToFinal: 900 });
    const res = await POST(request());
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, transcript: "Adebayo bought cement", msStopToFinal: 900 });
    const [pcm, opts] = transcribeStream.mock.calls[0];
    expect((pcm as Buffer).length).toBe(32000);
    expect(opts).toMatchObject({ apiKey: "test-key", language: "pcm", paceMs: 50 });
  });
  ...
  it("refuses a recording over the upload limit", async () => {
  ...
  it("maps a provider error to friendly text without leaking its detail", async () => {
    transcribeStream.mockRejectedValue(new IntronStreamError("secret upstream detail", "QUOTA_EXCEEDED", { recoverable: false }));
    const res = await POST(request());
  ...
  it("treats an empty transcript as no speech heard", async () => {
    transcribeStream.mockResolvedValue({ transcript: "   ", msStopToFinal: 500 });
    const res = await POST(request());
